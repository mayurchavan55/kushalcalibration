...
$(document).ready(function () {
  ...
  GetInstrumentMakeMaterial(function (data) {
    if (data) {
      data.instruments_master.forEach((item) => {
        ...
        $("#instrument_master").append(
          `<option value="${item.tim_id}">${item.tim_name}</option>`
        );
      });
      data.make_master.forEach((item) => {
        makeMap[item.tm_name] = item.tm_id;
        $("#make_manufacturer").append(
          `<option value="${item.tm_id}">${item.tm_name}</option>`
        );
      });
      data.material_master.forEach((item) => {
        materialMap[item.tmm_name] = item.tmm_id;
        $("#material").append(
          `<option value="${item.tmm_id}">${item.tmm_name}</option>`
        );
      });
      data.customer_master.forEach((item) => {
        customerMap[item.tcm_company_name] = item.tcm_id;
        $("#customer-name").append(
          `<option value="${item.tcm_id}">${item.tcm_company_name}</option>`
        );
      });
  ...
  $(".submit-jobregister").click(function () {
    ...
    validateField(
      ...
      "Please select the Instrument / Gauge."
    );
    validateField(
      "make_manufacturer",
      "make-manufacturer-error",
      "Please select the Make / Manufacturer."
    );
    validateField("material", "material-error", "Please select the Material.");
    validateField(
      "size-range",
      "size-range-error",
      "Please enter the Size / Range."
    );
    validateField(
      "lc-resolution",
      "lc-resolution-error",
      "Please enter the L.C. / Resolution."
    );
    validateField(
      "serial-no",
      "serial-no-error",
    ...
    validateField(
      "customer-id",
      "customer-id-error",
      "Please enter the Customer ID No."
    );
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response.status === true && response.data) {
        ...
        $("#symbol_id").val(
          "mm  ,m  ,in  ,mg  ,g  ,kg  ,°C  ,°F  ,min  ,A  ,V  ,Ω  ,µm  ,µ  ,cm² ,psi  , ± ,½"
        );
...
function excelImport() {
  fetch("/jobregisterExcelfileDownload")
    .then((res) => res.blob())
    .then((blob) => {
      const link = document.createElement("a");
      link.href = window.URL.createObjectURL(blob);
      link.download = "job_register.xlsx";
      link.click();
    })
